...
2026-09-24 사용자 검수에서 최소 길이를 15자에서 **8자**로 낮추기로 승인했다.
이 결정은 #22의 기존 최소 길이 정책을 대체하며, 관리자 bootstrap·가입·로그인·비밀번호
변경·재설정에 공통 적용한다. NFC 정규화 후 Unicode code point 기준 **8~128자**를 허용한다.
공백과 대소문자는 그대로 유지하며, 기존 비밀번호·hash는 변경하지 않는다.
