"""Create Model, Brand, Country, and Category Dimension slices.
...
def upgrade() -> None:
    for singular, plural in _DIMENSIONS:
        _create_dimension_table(plural)
        _create_log_table(singular, plural)
        _create_triggers(singular, plural)


def downgrade() -> None:
    for singular, plural in reversed(_DIMENSIONS):
        op.execute(
...
def _create_dimension_table(plural: str) -> None:
...
def _create_log_table(singular: str, plural: str) -> None:
